...
export function Sandbox() {
 ...
 const drawStars = () => {
   ...
   ctx.beginPath()
   ctx.strokeStyle = "rgba(163, 116, 255, 0.2)"
   ctx.lineWidth = 0.8
 ...
 }, [messages, width, height, isMobile, isTablet])
 ...
 return (
   <div ref={containerRef} className="relative w-full h-full bg-background overflow-hidden min-h-[300px]">
     ...
     <AnimatePresence>
       {hoveredMessage && (
         <motion.div
           ...
           exit={{ opacity: 0, y: 10 }}
           className={`absolute z-20 rounded-lg p-3 shadow-lg ${
         ...
         >
           <div className="flex items-center gap-2 mb-2">
             ...
             />
             <p className={`font-medium text-[#fffff3] ${isMobile ? 'text-sm' : ''}`}>
               {hoveredMessage.name}
             </p>
           </div>
           <p className={`text-[#fffff3]/90 leading-relaxed ${isMobile ? 'text-xs' : 'text-sm'}`}>
             {hoveredMessage.message}
           </p>
           <div className={`text-[#fffff3]/50 mt-3 pt-2 border-t border-[#A374FF]/20 ${
             isMobile ? 'text-[10px]' : 'text-xs'
           }`}>
             {new Date(hoveredMessage.created_at).toLocaleDateString()}
           </div>
           {isMobile && (
             <div className="text-[10px] text-[#fffff3]/40 mt-1 text-center">
               Tap to close
             </div>
           )}
         </motion.div>
     ...
     <Button
       ...
       className={`absolute ${isMobile ? 'bottom-4 right-4 h-12 w-12' : 'bottom-4 right-4'} rounded-full z-30 border-[#A374FF]/30 bg-black/50 hover:bg-[#A374FF]/20`}
       onClick={() => setShowForm(true)}
       style={{
         boxShadow: "0 0 10px rgba(163, 116, 255, 0.2)",
       }}
     >
       <Plus size={isMobile ? 18 : 20} className="text-[#fffff3]" />
     </Button>
     ...
     <Button
       variant="outline"
       size={isMobile ? "sm" : "icon"}
       className={`absolute ${isMobile ? 'bottom-4 left-4 h-12 w-12' : 'bottom-4 left-4'} rounded-full z-30 border-[#A374FF]/30 bg-black/50 hover:bg-[#A374FF]/20`}
       onClick={() => setShowInfo(!showInfo)}
       style={{
         boxShadow: "0 0 10px rgba(163, 116, 255, 0.2)",
       }}
     >
       <Info size={isMobile ? 16 : 18} className="text-[#fffff3]" />
     </Button>
     ...
     >
       <MessageSquare size={isMobile ? 12 : 14} className="text-[#A374FF]" />
     ...
     <AnimatePresence>
       {showInfo && (
         ...
         >
           <h4 className={`font-bold text-[#A374FF] mb-2 ${isMobile ? 'text-base' : ''}`}>
             About This Space
           </h4>
           <p className={`text-[#fffff3]/90 mb-3 ${isMobile ? 'text-sm' : 'text-sm'}`}>
           ...
           <p className={`text-[#fffff3]/90 ${isMobile ? 'text-sm' : 'text-sm'}`}>
         ...
         >
           ...
           >
             <div className="flex justify-between items-center mb-4">
               <h3 className={`font-bold text-[#fffff3] ${isMobile ? 'text-lg' : 'text-xl'}`}>
                 Add Your Message
               </h3>
               <Button
                 variant="ghost"
               ...
               >
                 <X size={isMobile ? 16 : 18} className="text-[#fffff3]" />
               </Button>
             </div>

             <form onSubmit={addMessage} className="space-y-4">
               <div className="space-y-2">
                 <label htmlFor="name" className={`font-medium text-[#fffff3] ${isMobile ? 'text-sm' : 'text-sm'}`}>
                   Your Name
                 </label>
                 <Input
                   id="name"
               ...
               <div className="space-y-2">
                 <label htmlFor="message" className={`font-medium text-[#fffff3] ${isMobile ? 'text-sm' : 'text-sm'}`}>
                   Your Message
                 </label>
                 <Textarea
                   id="message"
                   ...
                   maxLength={280}
                 />
                 <div className={`text-right text-[#fffff3]/60 ${isMobile ? 'text-xs' : 'text-xs'}`}>
                   {message.length}/280
                 </div>
               </div>
               ...
               <div className={`flex gap-3 pt-2 ${isMobile ? 'flex-col' : ''}`}>
                 <Button
                   type="button"
                   variant="outline"
                   className={`border-[#A374FF]/30 hover:bg-[#A374FF]/10 text-[#fffff3] ${
                 ...
                 <Button
                   type="submit"
                   className={`bg-[#A374FF] hover:bg-[#A374FF]/80 text-[#fffff3] ${
                     isMobile ? 'w-full' : 'flex-1'
                   }`}
