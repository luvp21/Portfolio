...
/* If you have existing Moon/Sun components, import them instead.
   I include tiny inline SVG components below as fallback. */
const Moon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg {...props} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className={props.className}>
...
/* ---------- helpers (getPositionCoords, generateSVG, getTransformOrigin, createAnimation) ---------- */
...
::view-transition-new(root) {
  mask: url('${safeUrl}') center / 0 no-repeat;
  animation: scale 3s;
}
...
@keyframes scale {
  ...
  100% {
    ...
    // circle with offset start positions (not center)
    if (variant === "circle" && start !== "center") {
